...
void main() {
    ...
    // -- * | 2 4 2 |
    // 16   | 1 2 1 |
    out_Color = vec4(vec3(e*4.0), 1);
    out_Color += vec4(vec3((b+d+f+h)*2.0), 0);
    out_Color += vec4(vec3(a+c+g+i), 0);
    out_Color *= vec4(vec3(1.0 / 16.0), 1);

    out_Color += texture(previousTexture, texCoord);
}