...
public class ShadowBox {
	...
	public Vector3f getCenter() {
		...
		Vector4f tmp = invertedLight.transform(cen);
	...
	private Vector4f[] calculateFrustumVertices(Matrix4f rotation, Vector3f forwardVector,
		...
		Vector4f tmp = new Vector4f(rotation.transform(UP));
		Vector3f upVector = new Vector3f(tmp.x, tmp.y, tmp.z);
		...
		Vector3f nearTop = new Vector3f(centerFar.add(new Vector3f(upVector.x * nearHeight,
	...
	private Vector4f calculateLightSpaceFrustumCorner(Vector3f startPoint, Vector3f direction, float width) {
		...
		point4f = new Vector4f(lightViewMatrix.transform(point4f));
		return point4f;
	}
